...
int array_index(const int nlist[], int len, int item){
    for(int i = 0; i < len; i++){
...
int check_if_valid_update(char input[], int *chp, pair *pairs, int pr_amt){
    ...
    while(reordered){
        ...
        for(int p = 0; p < pr_amt; p++){
            ...
            if(bef_idx > afr_idx){
                int tmp_update = updates[bef_idx];
                updates[bef_idx] = updates[afr_idx];
                updates[afr_idx] = tmp_update;
                reordered = 1;
            }
        }
        if(reordered) update_selected = 1;
    }
    if(!update_selected) return 0;
    if(len&1 != 1){

...
unsigned int find_longest(unsigned char** adj_mtrx, int n_cities, char *path, char depth, unsigned int curr_dist, unsigned int max_dist){
    if(depth == n_cities) 
        return curr_dist > max_dist ? curr_dist : max_dist;
    for(int c = 0; c < n_cities; c++){
        if(in_path(path,c,depth)) continue;
        path[depth] = c;
        if(depth){
            int prev_city = path[depth-1];
            max_dist = find_longest(adj_mtrx, n_cities, path, depth+1, curr_dist + adj_mtrx[prev_city][c],max_dist); 
        }else{
            max_dist = find_longest(adj_mtrx, n_cities, path, depth+1, 0, max_dist);
        }
        path[depth] = 0x80;
    }
    return max_dist;
...
int main(){
    ...
    unsigned int longest_path = find_longest(adj_mtrx,n_cities,path,0,0,0);
